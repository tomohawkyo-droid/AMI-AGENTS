...
from ami.hooks.types import HookResult
...
class TestBootloaderAgent:
    ...
    def test_execute_shell_safe(self, MockExecutor, agent):
    ...
    def test_execute_shell_blocked_by_hook(self, MockExecutor, agent):
        """execute_shell blocks commands when hook denies."""
        hook_manager = MagicMock()
        hook_manager.run.return_value = HookResult(
            allowed=False, message="Violation detected"
        )
