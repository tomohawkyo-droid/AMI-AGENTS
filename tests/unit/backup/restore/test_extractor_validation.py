...
class TestValidateArchive:
    ...
    @patch("ami.scripts.backup.restore.extractor._validate_full_tar")
    @patch("ami.scripts.backup.restore.extractor._validate_tar_sample")
    @patch("ami.scripts.backup.restore.extractor.zstd")
    @patch("builtins.open")
    @patch("pathlib.Path.exists")
...
class TestGetZstdBinaryFindBootstrapped:
    ...
    @patch("ami.scripts.backup.restore.extractor.Path")
    def test_get_zstd_binary_finds_bootstrapped(self, mock_path_class):
        ...
        def truediv_side_effect(x):
            if x == "pyproject.toml":
                ...
                boot_mock = MagicMock()
                boot_mock.__truediv__ = (
                    lambda _, y: MagicMock(__truediv__=lambda _, z: mock_zstd_bin)
                    if y == "bin"
                    else MagicMock()
                )
                return boot_mock
