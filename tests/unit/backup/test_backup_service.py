...
class TestBackupService:
    ...
    async def test_run_backup_accepts_options(self, tmp_path):
        ...
        with patch(
            "ami.scripts.backup.create.service.create_zip_archive"
        ) as mock_archive:
            mock_archive.return_value = tmp_path / "test-backup.tar.zst"
            (tmp_path / "test-backup.tar.zst").write_bytes(b"fake archive")

            with patch(
                "ami.scripts.backup.create.service.copy_to_secondary_backup"
            ) as mock_secondary:
                mock_secondary.return_value = None

                with patch(
                    "ami.scripts.backup.create.service.cleanup_local_zip"
                ) as mock_cleanup:
