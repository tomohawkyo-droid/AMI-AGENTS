...
from ami.core.conversation import ConversationEntry, EntryOrigin, EntryRole
...
class TestTranscriptSearcher:
    ...
    def _populate_store(self, tmp_path: Path) -> StoreWithSession:
        ...
        store.add_entry(
            sid,
            _make_entry(role=EntryRole.USER, text="deploy the application"),
        )
        store.add_entry(
            sid,
            _make_entry(
                role=EntryRole.ASSISTANT, text="application deployed successfully"
            ),
        )
        store.add_entry(
            sid,
    ...
    def test_search_skips_empty_entries(self, tmp_path: Path):
        ...
        store.add_entry(
            sid,
            _make_entry(role=EntryRole.ASSISTANT, text=""),
        )
        searcher = TranscriptSearcher(store)
