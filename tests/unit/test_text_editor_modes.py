...
class TestHandleNavigationCommandKeys:
    ...
    def test_submit_returns_true(self) -> None:
        """Test SUBMIT (Ctrl+S) returns True to exit."""
        editor = TextEditor()
        display = MagicMock()

        result = editor._handle_navigation_command_keys("SUBMIT", display)
    ...
    def test_alt_enter_adds_newline_via_alt(self) -> None:
        """Test ALT_ENTER adds newline (duplicate coverage with different name)."""
        editor = TextEditor("hello")
        editor.cursor_manager.current_col = 5
        display = MagicMock()
