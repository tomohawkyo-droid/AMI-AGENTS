...
def interactive_guard_rules():
    ...
    agents_root = _find_project_root()
    return agents_root / "ami/config/policies/interactive.yaml"


def test_static_command_guard_blocks_dangerous_commands(
    agent_fixture, interactive_guard_rules
):
    ...
    result = agent_fixture.execute_shell(
        script, guard_rules_path=interactive_guard_rules
    )
...
def test_static_command_guard_allows_whitelisted_commands(
    agent_fixture, interactive_guard_rules
):
