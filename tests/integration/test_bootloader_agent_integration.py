...
from ami.core.conversation import ConversationTranscript
...
class TestExecuteShell:
    ...
    def test_blocked_dangerous_command(self, agent: BootloaderAgent):
        ...
        hook_manager = HookManager.from_config(
            get_project_root() / "ami/config/hooks.yaml"
        )
        output = agent.execute_shell("rm -rf /", hook_manager=hook_manager)
...
class TestExecuteShellBlocks:
    ...
    def test_empty_blocks(self, agent: BootloaderAgent, tmp_path: Path):
        ...
        agent._execute_shell_blocks([], ctx, parts, hook_manager, transcript)
        # No entries added for empty blocks
        assert len(transcript.entries) == 0

    def test_stop_event_terminates(self, agent: BootloaderAgent, tmp_path: Path):
        store = TranscriptStore(root=tmp_path)
        sid = store.create_session(provider="test", model="m")
        transcript = ConversationTranscript(store, sid)
        ...
        parts: list[str] = []
        hook_manager = HookManager.noop()
        agent._execute_shell_blocks(["echo hi"], ctx, parts, hook_manager, transcript)
        # Stop event means no commands executed, no entries added
        assert len(transcript.entries) == 0
