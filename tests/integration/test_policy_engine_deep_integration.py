...
class TestBashPatterns:
    """Test bash pattern loading."""

    def test_load_default_bash_patterns(self):
        engine = PolicyEngine()
        patterns = engine.load_bash_patterns("default")
        assert isinstance(patterns, list)
        assert len(patterns) > 0

    def test_bash_patterns_have_structure(self):
        engine = PolicyEngine()
        patterns = engine.load_bash_patterns("default")
        if patterns:
            first = patterns[0]
    ...
    def test_load_interactive_bash_patterns(self):
        engine = PolicyEngine()
        patterns = engine.load_bash_patterns("interactive")
        assert isinstance(patterns, list)
        assert len(patterns) > 0

    def test_nonexistent_name_returns_empty(self):
        engine = PolicyEngine()
        patterns = engine.load_bash_patterns("nonexistent_policy_xyz")
        assert patterns == []

    def test_caching_returns_same_list(self):
        engine = PolicyEngine()
        first = engine.load_bash_patterns("default")
        second = engine.load_bash_patterns("default")
...
class TestGetPolicyPath:
    ...
    def test_valid_path(self):
        engine = PolicyEngine()
        path = engine._get_policy_path("bash", "default")
        if path is not None:
    ...
    def test_single_level_key(self):
        engine = PolicyEngine()
        # "bash" is a dict, not a string -- should return None
        path = engine._get_policy_path("bash")
        # If bash is a dict of entries, single key should not resolve
...
class TestLogicLoaders:
    ...
    def test_load_bash_patterns_default(self):
        patterns = load_bash_patterns()
        assert isinstance(patterns, list)
        assert len(patterns) > 0

    def test_load_bash_patterns_custom_path(self, project_root: Path):
        custom = project_root / "ami/config/policies/interactive.yaml"
        if custom.exists():
            patterns = load_bash_patterns(custom)
            assert isinstance(patterns, list)
            assert len(patterns) > 0
