...
def select_backup_interactive(backup_files: List[Dict[str, Any]]) -> Optional[str]:
    ...
    selector = MenuSelector(menu_items, "Select a backup to restore", max_visible_items=10)
    selected = selector.run()
    
    if selected:
        selected_item = selected[0]
        logger.info(f"Selected backup: {selected_item.label} (ID: {selected_item.value})")
        return selected_item.value
        
    return None


def display_backup_list(backup_files: List[Dict[str, Any]], title: str = "Available Backup Files"):
    """
    ...
    for i, file_info in enumerate(backup_files, 1):
        ...
        print(f"    {Colors.YELLOW}Modified:{Colors.RESET} {modified_time} {Colors.CYAN}|{Colors.RESET} {Colors.YELLOW}Size:{Colors.RESET} {size_str}")
        print(f"    {Colors.YELLOW}File ID:{Colors.RESET} {file_id[:8]}...{file_id[-4:] if len(file_id) > 12 else file_id}")
    
    print(f"{Colors.CYAN}└{'─' * 78}┘{Colors.RESET}")


def select_backup_by_index(backup_files: List[Dict[str, Any]], index: int) -> Optional[str]:
    """
    ...
    if 0 <= index < len(backup_files):
        ...
        logger.error(f"Invalid backup index: {index}. Available backups: {len(backup_files)}")
        return None
