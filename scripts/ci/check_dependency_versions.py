...
class DependencyConfig(TypedDict, total=False):
    ...
    base_toml_path: str
    generated_toml_path: str
    forbidden_packages: list[str]


DEFAULT_CONFIG: DependencyConfig = {
...
def load_config(
    config_path: str = "res/config/dependency_check.yaml",
) -> DependencyConfig:
    if Path(config_path).exists():
        with open(config_path) as f:
            return cast(DependencyConfig, yaml.safe_load(f))
...
def parse_dependencies(path: Path) -> list[str]:
    ...
    parsed = []
    for dep in deps:
        if dep.strip() == "# VENDOR_DEPENDENCIES_MARKER":
            continue
